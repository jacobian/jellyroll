...
class TracTest(TestCase):
    ...
    def testProject(self):
        p = TracProject.objects.get(pk=1)
        self.assertEqual(p.rss_url, "http://code.djangoproject.com/timeline?ticket=on&changeset=on&wiki=on&max=100&daysback=60&format=rss")
        
    def testCommit(self):
        c = TracCommit.objects.get(pk=1)
...
class ItemTest(TestCase):
    ...
    def testEverything(self):
        self.assertEqual(Item.objects.count(), 11)
